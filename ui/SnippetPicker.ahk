...
class SnippetPicker {
    ...
    static _BuildGui() {
        ...
        Hotkey("Enter", (*) => this._InsertSelected(), "On")
    ...
    static _ShowEditGui() {
        ...
        editGui.Add("Button", "x+5 w70", "修正").OnEvent("Click", (*) => (
            (r := lv.GetNext()) ? this._ShowEntryGui(editGui, lv, r) : this._ModalAction(editGui, () => MsgBox(
                "修正する行を選択してください。", "警告", "Icon! " this.FLAG_MODAL))
        ))
    ...
    static _ShowEntryGui(parentGui, lv, row := 0) {
        ...
        grpEd := entryGui.Add("Edit", "w400 vGroup", row ? lv.GetText(row, 1) : "Default")
        entryGui.Add("Text", , "名称:")
        nameEd := entryGui.Add("Edit", "w400 vName", row ? lv.GetText(row, 2) : "")
        entryGui.Add("Text", , "内容:")
        contEd := entryGui.Add("Edit", "w400 r10 vContent", row ? lv.GetText(row, 3) : "")
    ...
    static _ProcessEntry(parentGui, entryGui, lv, row) {
        ...
        if (res.Group == "" || res.Name == "" || res.Content == "") {
            this._ModalAction(entryGui, () => MsgBox("すべての項目を入力してください。", "エラー", "Icon! " this.FLAG_MODAL))
            return
        }
    ...
    static _DeleteItem(lv, editGui) {
        ...
        if (selectedRows.Length == 0) {
            this._ModalAction(editGui, () => MsgBox("削除する行を選択してください。", "警告", "Icon! " this.FLAG_MODAL))
            return
        }

        confirmMsg := (selectedRows.Length == 1) ? "選択した項目を削除しますか？" : selectedRows.Length " 件の項目を一括削除しますか？"
        ans := this._ModalAction(editGui, () => MsgBox(confirmMsg, "削除の確認", "YesNo Icon? " this.FLAG_MODAL))

        if (ans == "Yes") {
            loop selectedRows.Length {
                targetRow := selectedRows.Pop()
                lv.Delete(targetRow)
            }
        }
    }
    ...
    static _InsertSelected() {
        ...
        dispG := this.LvObj.GetText(row, 1)
        dispT := this.LvObj.GetText(row, 2)
        for item in this.SnipList {
